<<<<<<< HEAD
=======
### modulue: Stokes shift & related quantities within configuration coordinate model
### H. Saito, 8th, Mar., 2024
...
=======
        (alat_g, plat_g, elements_g, nelems_g, natm_g, pos_g) = subs.get_POSCAR("POSCAR_"+prms.stateg)
        (alat_e, plat_e, elements_e, nelems_e, natm_e, pos_e) = subs.get_POSCAR("POSCAR_"+prms.statee)
>>>>>>> 2482b5ac0daf49ca16f8c4b31e4e33b5393f7357
...
### ----------------------------------------------------------------------------- ###
def fit_Ecurve(state: str, deltaQ: float, EFC: float, sw_unit: str):
    """ fiting Energy curve by polynomial """
    print("* --- Fitting energy curve --- *")
    fn: str = "DATA_"+state+".dat"
    Q, Etot = np.loadtxt(fn,dtype='float',unpack=True,ndmin=0)
    x: float = np.linspace(-0.1+Q[0],0.1+Q[len(Q)-1],100)
    weight: float = np.ones(len(Q))
    coef_3dim: float = np.polyfit(Q, (Etot-min(Etot))*Ry, 3, w=weight)
    Efit_3dim: float = np.poly1d(coef_3dim)(x)
    coef_2dim: float = np.polyfit(Q, (Etot-min(Etot))*Ry, 2, w=weight)
    Efit_2dim: float = np.poly1d(coef_2dim)(x)

    # consider Anharmonic effects
    cfunit: float = hbar*1.e10*np.sqrt(1./(ep*uatm))
    Qmin_3dim: float = (-coef_3dim[1]+np.sqrt(coef_3dim[1]**2.-3.*coef_3dim[0]*coef_3dim[2]))/(3.*coef_3dim[0])
    Omega_3dim: float = cfunit*np.sqrt(6.*coef_3dim[0]*Qmin_3dim+2.*coef_3dim[1])
    S_3dim: float = EFC / Omega_3dim
    print("* Fitted by cubic equation: ")
    ...
    plt.xlabel(r"Q ($\sqrt{\mathrm{amu}} \cdot \AA$)")
    plt.ylabel("Energy (eV)")
    plt.scatter(Q,(Etot-min(Etot))*Ry,marker="o",label="data",edgecolor="darkred",color="white",s=30)
    plt.plot(x,Efit_2dim,linestyle="dashed",color="mediumblue",label="fit 2dim")
    plt.scatter(Qmin_2dim, np.poly1d(coef_2dim)(Qmin_2dim), marker="*", color="darkblue", s=60)
    plt.plot(x,Efit_3dim,linestyle="dashed",color="coral",label="fit 3dim")
    plt.scatter(Qmin_3dim, np.poly1d(coef_3dim)(Qmin_3dim), marker="*", color="crimson", s=60)
    ...
    plt.savefig("Ecurve_fit_"+state+".pdf")
    plt.show()
    return (Qmin_2dim, Omega_2dim, S_2dim, Qmin_3dim, Omega_3dim, S_3dim)

### ----------------------------------------------------------------------------- ###
def run_stokes():
    """ execute stokes calculation program """
    ...
    if ( sw_unit in {"eV","nm"} ):
        ...
        unit: float = eV2cm
    (DeltaS, EFCe, EZPL) = get_Stokes(Eabs0, Eem0, EFCg)
    (DeltaR, DeltaQ, Omegag, Omegae, Sabs, Sem) = get_DeltaQ(EFCg, EFCe, ndiv_eg, sw_eg)
    if ( sw_eg ):
        calc_Eeg(mat,nc,exe,stateg,DeltaQ,dq=dQ,neg=ndiv_eg)
        calc_Eeg(mat,nc,exe,statee,DeltaQ,dq=dQ,neg=ndiv_eg)
        if ( sw_plt ):
            (Qming_2dim,Omegag_2dim,Sem_2dim,Qming_3dim,Omegag,Sem) = fit_Ecurve(stateg,DeltaQ,EFCg,sw_unit)
            (Qmine_2dim,Omegae_2dim,Sabs_2dim,Qmine_3dim,Omegae,Sabs) = fit_Ecurve(statee,DeltaQ,EFCe,sw_unit)
        
    (energy, Lem, Labs) = Line_shape(ndive, nmax, I0, Sem, Sabs, EZPL, Omegag, Omegae, sw_unit, emin_plt, emax_plt)
    (temp_sp, Eabs, Eem) = Ecenter_shift(ndivtemp, Eabs0, Eem0, Omegag, Omegae, DeltaS)
    (temp_w, W0, W) = FWHM(ndivtemp, Eem0, Sabs, Sem, Omegag, Omegae, sw_unit)
    if ( sw_unit in {"eV","cm^-1"} ):
        print("* Absorption energy ({sunit}): ".format(sunit=sw_unit), Eabs0*unit)
        print("* Emission energy ({sunit}): ".format(sunit=sw_unit), Eem0*unit)
        print("* EFCg = Ee - Eg ({sunit}): ".format(sunit=sw_unit), EFCg*unit)
        print("* EFCe = Eg* - Ee* ({sunit}): ".format(sunit=sw_unit), EFCe*unit)
        print("* Stokes shift (@ 0K) ({sunit}): ".format(sunit=sw_unit), DeltaS*unit)
        print("* Zero phonon photoemission line ({sunit}): ".format(sunit=sw_unit), EZPL*unit)
        print("* FWHM (@ 0K) ({sunit}): ".format(sunit=sw_unit), W0*unit)
        print("* DeltaR (ang): ", DeltaR)
        print("* DeltaQ (sqrt(amu)*ang): ", DeltaQ)
        print("* hbar*Omegag ({sunit}): ".format(sunit=sw_unit), Omegag*unit)
        print("* hbar*Omegae ({sunit}): ".format(sunit=sw_unit), Omegae*unit)
    elif ( sw_unit == "nm" ):
        print("* Absorption energy (nm): ", E2lambda(Eabs0))
        print("* Emission energy (nm): ", E2lambda(Eem0))
        print("* EFCg = Ee - Eg (nm): ", E2lambda(EFCg))
        print("* EFCe = Eg* - Ee* (nm): ", E2lambda(EFCe))
        print("* Stokes shift (@ 0K) (nm): ", E2lambda(DeltaS))
        print("* Zero phonon photoemission line (nm): ", E2lambda(EZPL))
        print("* FWHM (@ 0K) (nm): ", E2lambda(W0))
        print("* DeltaR (ang): ", DeltaR)
        print("* DeltaQ (sqrt(amu)*ang): ", DeltaQ)
        print("* hbar*Omegag (nm): ", E2lambda(Omegag))
        print("* hbar*Omegae (nm): ", E2lambda(Omegae))
    else:
        print("* ERROR: sw_unit should be 'eV', 'nm', or 'cm^-1'!!!")
        sys.exit()
    ...
    if sw_plt:
        ...
        plt.axhline(0, lw=0.5, c="black", linestyle="dashed")
        if ( sw_unit in {"eV","cm^-1"} ):
            plt.xlabel("Energy ({sunit})".format(sunit=sw_unit))
            plt.plot(unit*energy, Labs, lw=1.0, c="black")
            plt.scatter(Eabs0*unit,0.0,c="black",s=size,marker="o")
            plt.plot(unit*energy, Lem, lw=1.0, c="red")
            plt.scatter(Eem0*unit,0.0,c="red",s=size,marker="o")
        elif ( sw_unit == "nm" ):
            plt.xlabel("Wave length (nm)")
            plt.plot(E2lambda(energy), Labs, lw=1.0, c="black")
            plt.plot(E2lambda(energy), Lem, lw=1.0, c="red")
        plt.savefig("Spectrum.pdf")
        plt.show()
        
        print("* --- PLOT TEMPERATURE DEPENDENCE OF E_EM & E_ABS --- *")
        plt.xlabel("Temperature (K)")
        plt.ylabel("Emission energy ({sunit})".format(sunit=sw_unit))
        if ( sw_unit in {"eV","cm^-1"} ):
            plt.plot(temp_sp, unit*Eabs, lw=1.0, c="black")
            plt.plot(temp_sp, unit*Eem, lw=1.0, c="red")
        elif ( sw_unit == "nm" ):
            plt.plot(temp_sp, E2lambda(Eabs), lw=1.0, c="black")
            plt.plot(temp_sp, E2lambda(Eem), lw=1.0, c="red")
        plt.savefig("Epeak_Temp.pdf")
        plt.show()
        
        print("* --- PLOT TEMPERATURE DEPENDENCE OF STOKES SHIFT --- *")
        plt.xlabel("Temperature (K)")
        plt.ylabel("Stokes shift ({sunit})".format(sunit=sw_unit))
        if ( sw_unit in {"eV","cm^-1"} ):
            plt.plot(temp_sp, unit*(Eabs-Eem), lw=1.0, c="black")
        elif ( sw_unit == "nm" ):
            plt.plot(temp_sp, E2lambda(Eabs)-E2lambda(Eem), lw=1.0, c="black")
        plt.savefig("Stokes_Temp.pdf")
        plt.show()
        
        print("* --- PLOT TEMPERATURE DEPENDENCE OF FWHM --- *")
        plt.xlabel("Temperature (K)")
        plt.ylabel("FWHM ({sunit})".format(sunit=sw_unit))
        plt.plot(temp_w, unit*W, lw=1.0, c="black")
        plt.savefig("FWHM_Temp.pdf")
        plt.show()
    else:
        print("* --- NO PLOT --- *")
